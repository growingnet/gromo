...
- Add a `to` method to the `GrowingContainer`, `GrowingModule` and `TensorStatistic` classes to move the module to a different device and/or dtype. (:gh:`93` by `Stéphane Rivaud`_)
