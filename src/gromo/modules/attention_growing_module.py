...
from gromo.utils.my_utils import my_svd_low_rank
...
class PrototypeAttention(nn.Module):
    ...
    def get_growed_weight_matrices(
        self, X: torch.Tensor, add_bias_before_pseudoinverse: bool, p: int = 1
    ):
        """
        p: neuron to add
        """
        ...
        breve_W_Q, breve_W_K, reconstruction_error = my_svd_low_rank(
            Z_mean, self.d_k + p, reconstruction_error=True
        )
...
if __name__ == "__main__":
    ...
    W_Q, W_K, rel_err = model.get_growed_weight_matrices(x, False)
