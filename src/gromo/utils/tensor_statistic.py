...
class TensorStatistic:
    ...
    def update(self, **kwargs):
        if self.updated is False:
            ...
            assert self._shape == update.size(), f"The update tensor has a different size than the tensor statistic {self.name}"f" {update.size()=}, {self._shape=}"
...
class TensorStatisticWithError(TensorStatistic):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.trace_precision = kwargs.get("trace_precision", 1e-3) # relative precision stopping criterion for the trace computation
    ...
    def error(self):
        ...
        if self._compute_trace:
            warn(f"The desired trace precision has not been reached for the tensor statistic {self.name}. The error estimate may be inaccurate.")
    ...
    def update(self, **kwargs):
        if self.updated is False:
            ...
            if self._compute_trace:
                ...
                trace_covariance = (self._square_norm_sum ) / (self._batches) - mu_n_norm**2
                if self._trace is not None:
                    delta_trace_covariance = trace_covariance - self._trace
                    if abs(delta_trace_covariance) < self.trace_precision * abs(trace_covariance):
