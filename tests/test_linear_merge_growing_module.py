...
class TestLinearMergeGrowingModule(TorchTestCase):
    ...
    def test_input_storage(self, bias: bool):
        ...
        self.assertEqual(
            demo_layers["add"].store_activity,
            1,
            "Merge module should forward store_activity requests",
        )
        self.assertFalse(
            demo_layers["next"]._internal_store_input,
            "Internal store flag should be False when set via property",
        )
        self.assertIsNone(
            demo_layers["next"].input, "Input should be None before forward pass"
        )
    ...
    def test_activity_storage(self, bias: bool):
        ...
        # Verify initial state
        self.assertEqual(
            demo_layers["add"].store_input,
            1,
            "Merge module should forward store_input requests",
        )
        self.assertFalse(
            demo_layers["prev"]._internal_store_pre_activity,
            "Internal store flag should be False when set via property",
        )
        self.assertIsNone(
    ...
    def test_set_previous_next_modules(self, bias: bool):
        ...
        new_input_layer = LinearGrowingModule(
            2,
            3,  # 2 input features, 3 output features
            use_bias=bias,
            name="new_prev",
            device=global_device(),
